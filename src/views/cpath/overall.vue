  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="开始时间" width="200" sortable prop="date" align="center">
        <template slot-scope="scope">{{ scope.row.beginDate + " " + scope.row.beginTime }}</template>
      </el-table-column>
      <el-table-column label="结束时间" width="200" sortable prop="date" align="center">
        <template slot-scope="scope">{{ scope.row.endDate + " " + scope.row.endTime }}</template>
      </el-table-column>
      <el-table-column class-name="status-col" label="状态" prop="status" width="300" align="center">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            @click="fetchPrint(scope.row); dialog1 = true"
            type="success"
            size="small"
          >查看</el-button>
        </template>
      ...
      <el-table-column align="center" fixed="right" label="操作" width="300">
        ...
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.beginDate !== '---' && scope.row.status === '正在进行'"
          ...
          <el-collapse v-model="activeNames" @change="handleChange">
            ...
            <el-collapse-item title="临时医嘱" name="2">
              <div class="text-wrapper" v-for="(item,i) in checkedCities.city2">
                ...
                <div v-if="item === '常规检查'">
                  <div
                    v-for="(item2) in detailCities.city2.input0"
                    class="subtitle"
                  >{{ " ：" + item2 + " " }}</div>
                </div>
                <div v-if="item === '功能性检查'">
                  <div
                    v-for="(item2) in detailCities.city2.input1"
                    class="subtitle"
                  >{{ " ：" + item2 + " " }}</div>
                </div>
                <div v-if="item === '射线检查'">
                  <div
                    v-for="(item2) in detailCities.city2.input2"
                    class="subtitle"
                  >{{ " ：" + item2 + " " }}</div>
                </div>
          ...
          <el-collapse v-model="activeNames" @change="handleChange">
            ...
            <el-collapse-item title="长期医嘱" name="1">
              <div class="text-wrapper" v-for="(item,i) in checkedCities.city1">
                <div v-if="item != null" class="title">{{ item }}</div>
                <div
                  v-if="item === '苯丁酸氮芥10mg/(m^2*d)'"
                  class="subtitle"
                >{{ " ：" + detailCities.city1.input1 + "天" }}</div>
                <div v-if="item === '其他医嘱'" class="subtitle">{{ " ：" + detailCities.city1.input0 }}</div>
                <div
                  v-if="item === '利妥昔单抗'"
                  class="subtitle"
                >{{ " ：" + detailCities.city1.input2 + "mg/m2，1天" }}</div>
                <div
                  v-if="item === '氟达拉滨'"
                  class="subtitle"
                >{{ " ：" + detailCities.city1.input3 + "mg/(m^2*d)" + " " + detailCities.city1.input4 + "天" }}</div>
                <div
                  v-if="item === '环磷酰胺'"
                  class="subtitle"
                >{{ " ：" + detailCities.city1.input5 + "mg/(m^2*d)" + " " + detailCities.city1.input6 + "天" }}</div>
                <div
                  v-if="item === '甲泼尼龙1g/(m^2*d)'"
                  class="subtitle"
                >{{ " ：" + detailCities.city1.input7 + "天" }}</div>
              </div>
            </el-collapse-item>
            <el-collapse-item title="临时医嘱" name="2">
              <div class="text-wrapper" v-for="(item,i) in checkedCities.city2">
